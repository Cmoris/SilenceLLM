...
class PerceiverIO(nn.Module):
    ...
    def forward(
        ...
        return self.to_logits(latents), latents
...
class PerceiverLM(nn.Module):
    ...
    def forward(
        ...
        x = x + pos_emb
        logits, latents = self.perceiver_io(input_embeds, mask = mask, queries = x)
...
if __name__ == "__main__":
    ...
    seq = torch.randint(0, 20000, (1, 50))
    mask = torch.ones(1, 50).bool()
    input_embeds = torch.ones(1,50,1024)

    logits, latents = model(x=seq, input_embeds=input_embeds, mask = mask) 
    print(latents.size())