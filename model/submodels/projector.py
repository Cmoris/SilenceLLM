...
def load_mm_projector(model_path, cache_dir=None, token=None):
    ...
    mm_projector_weights = {k: v.to(torch.float32) for k, v in mm_projector_weights.items()}
...
def build_vision_projector(config, delay_load=False, **kwargs):
    projector_type = getattr(config, 'mm_projector_v_type', 'linear')
    ...
    if projector_type == "linear":
        ...
        return Linear(config.mm_hidden_size_v, config.mm_hidden_size_v)
...
class MLP(nn.Module):
    def __init__(self, input_dim, out_dim, mlp_depth):
        super().__init__()
...
class STCConnector(nn.Module):
    ...
    def __init__(self, config, downsample=(2, 2, 2), depth=4, mlp_depth=2):
        ...
        """
        super().__init__()
        self.encoder_hidden_size = encoder_hidden_size = config.mm_hidden_size_v
        self.hidden_size = hidden_size = config.mm_hidden_size_v
        self.output_hidden_size = output_hidden_size = config.mm_hidden_size_v
    ...
    def forward(self, x):
        ...
        x = self.readout(x)
        return x # torch.Size([1, 2704, 3584])
