...
class SilencePerceiverConfig(PretrainedConfig):
    model_type = "silence_perceiver"
    ...
    def __init__(self, 
        ...
        self.model_type = "silence_perceiver"
...
class SilencePerceiverModel(SilenceMetaModel, PreTrainedModel):
    config_class = SilencePerceiverConfig
    
    def __init__(self, config):
        super(SilencePerceiverModel, self).__init__(config)


class SilencePerceiverForCausalLM(PreTrainedModel, SilenceMetaForCausalLM):
    config_class = SilencePerceiverConfig
    
    def __init__(self, config, **kwargs):
        super().__init__(config)
        self.model = SilencePerceiverModel(config)
        self.perceiver = PerceiverLM(
            num_tokens = config.vocab_size,          # number of tokens
            dim = config.hidden_size,                    # dimension of sequence to be encoded
            depth = config.depth,                   # depth of net
            max_seq_len = config.max_seq_len,          # maximum sequence length
            num_latents = config.num_latents,           # number of latents, or induced set points, or centroids. different papers giving it different names
            latent_dim = config.latent_dim,            # latent dimension
            cross_heads = config.cross_heads,             # number of heads for cross attention. paper said 1
            latent_heads = config.latent_heads,            # number of heads for latent self attention, 8
            cross_dim_head = config.cross_dim_head,         # number of dimensions per cross attention head
            latent_dim_head = config.latent_dim_head,        # number of dimensions per latent self attention head
            weight_tie_layers = config.weight_tie_layers    # whether to weight tie layers (optional, as indicated in the diagram)
    ...
    def forward(self, source) -> CausalLMOutput:
        ...
        if self.config.use_qformer and av_feat is not None:
            max_seq_len = max(len_queries)
            batch_size = av_feat.size(0)
            ...
            attention_mask = torch.zeros(batch_size, max_seq_len, dtype=torch.bool, device=query_output.device)
        else:
            # Directly map fused AV features to LLM embedding space
            query_output = self.get_model().mm_projector(av_feat)
            attention_mask = torch.zeros(batch_size, max_seq_len, dtype=torch.bool, device=query_output.device)

        logits, latents = self.perceiver(x=source["input_ids"], input_embeds=query_output, mask = attention_mask)
        hidden_states = self.fc(latents.mean(1))
